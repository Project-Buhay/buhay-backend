...
async def save_route(input: SaveRouteInput):
    try:
        ...
        for i in range(t):
            ...
            start_name = gmaps.reverse_geocode(
                (start_lat, start_lng),
                result_type="street_address|plus_code|premise|establishment|point_of_interest",
            )[0]["formatted_address"]
            end_name = gmaps.reverse_geocode(
                (end_lat, end_lng),
                result_type="street_address|plus_code|premise|establishment|point_of_interest",
            )[0]["formatted_address"]

            coordinate_names.append(start_name)
        ...
        success = await update_route_info_id(
            input.request_id, route_id, coordinate_names
        )
        return {
            "success": (
                True if success is not None and success == input.request_id else False
            )
        }
